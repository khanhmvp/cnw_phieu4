...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            // Kiểm tra mã lỗi trùng khóa UNIQUE (23000 hoặc mã lỗi MySQL 1062)
            if ($e->getCode() == '23000' || (isset($e->errorInfo[1]) && $e->errorInfo[1] == 1062)) {
                $errors[] = 'Tên danh mục "' . htmlspecialchars($name, ENT_QUOTES, 'UTF-8') . '" đã tồn tại!';
            } else {
                // In ra chi tiết lỗi nếu muốn debug: $e->getMessage()
...
<head>
    ...
    <style>
        ...
        .btn { padding: 6px 12px; background-color: #0275d8; color: white; border: none; cursor: pointer; border-radius: 4px; }
    </style>
</head>
<body>
